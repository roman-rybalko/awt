...
<xsl:text/>Subject: [Report] Scheduled Job "<xsl:value-of select="@sched_name"/>" start FAILED
</xsl:template>
...
Scheduled Job: "<xsl:value-of select="@sched_name"/>" (<xsl:value-of select="../@root_url"/>?schedule=1#<xsl:value-of select="@sched_id"/>)
