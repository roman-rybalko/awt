...
From: =?UTF-8?B?0J/RgNC+0YTQktC10LHQotC10YHRgg==?= &lt;<xsl:value-of select="@from"/>&gt;
To: <xsl:value-of select="php:function('composer_mime_encode', string(@to))"/>
Message-Id: &lt;<xsl:value-of select="@message_id"/>&gt;
Date: <xsl:value-of select="@date"/>
List-Unsubscribe: &lt;mailto:support@webmonit.ru?subject=Mail%20Unsubscribe%20Request:%20&amp;body=Login:%20<xsl:value-of select="*/@login"/>%0aEmail:%20<xsl:value-of select="php:function('composer_url_encode', string(@to))"/>%0a&gt;
