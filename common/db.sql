...
create table tasks(user_id integer not null, test_id integer not null, type varchar(255) not null, debug integer(1), status integer not null, data varchar(255), task_id integer primary key auto_increment not null, time integer not null);
create index tasks_idx on tasks(status); -- первоначальный запрос
create index tasks_idx2 on tasks(time); -- удаление
create table task_actions(task_id integer not null, type varchar(255) not null, selector varchar(255), data varchar(255), action_id integer not null, scrn_filename varchar(255), failed varchar(256));
